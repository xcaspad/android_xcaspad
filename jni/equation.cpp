...
eqwdata Equation::Equation_total_size(const gen & g){
  if (g.type==_EQW)
    return *g._EQWptr;
...
void Equation::Equation_vertical_adjust(int hp,int yp,int & h,int & y){
  int yf=min(y,yp);
  h=max(y+h,yp+hp)-yf;
  y=yf;
}

vecteur Equation::Equation_subsizes(const gen & arg,const attributs & a,int windowhsize,GIAC_CONTEXT){
  vecteur v;
...
void Equation::Equation_translate(gen & g,int deltax,int deltay){
  if (g.type==_EQW){
    g._EQWptr->x += deltax;
    g._EQWptr->y += deltay;
...
gen Equation::Equation_compute_symb_size(const gen & g,const attributs & a,int windowhsize,GIAC_CONTEXT){
  if (g.type!=_SYMB)
...
gen Equation::Equation_compute_size(const gen & g,const attributs & a,int windowhsize,GIAC_CONTEXT){
  fl_font(FL_HELVETICA,a.fontsize);
  /**************
   * FL_WIDGET  *
   **************/
...
void Equation::Equation_draw(const eqwdata & e,int x,int y,int rightx,int lowery,Equation * eq,int begin_sel,int end_sel){
...
void Equation::Equation_draw(const gen & g,int x,int y,int rightx,int lowery,Equation * equat){
  const giac::context * contextptr = 0;//get_context(equat);
  ...
  if (x0<rightx){
    ...
    fl_arc(eqx+x0,eqy+y0+fontsize/3,fontsize/3,fontsize/3,180,360);//adjust for xcas pad
...
Fl_Font Equation::cst_greek_translate(string & s0){
  string s1(s0);
  s0="";
  int n=s1.size();
  for (int i=0;i<n;++i){
